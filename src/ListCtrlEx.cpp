...
int CALLBACK CListCtrlEx::Compare(LPARAM lParam1, LPARAM lParam2, LPARAM lParam)
{
    ...
    unsigned char Type = SortInfo.SortType;
    int  sOrder = (SortInfo.Ascending != 0)? 1 : -1; 
...
void CListCtrlEx::SortByColumn( int nColumn, BOOL bAscending )
{
    nSortedCol = nColumn;
...
void CListCtrlEx::DefaultSort()
{
    if (nSortedCol == -1) return;

    int NumItems = GetItemCount();  
    ListData* pData = new ListData[NumItems];
    ...
    for (int i = 0; i < NumItems; i++)  
    {
        ...
        pData[i].dwOldData = dwData;
        pData[i].strText = strText;
        pData[i].BaseTime = m_ctStart;
        
        SetItemData(i, (DWORD)&pData[i]);    
    }

    DWORD nSortType;

    if (m_pTypes)
        nSortType = m_pTypes[nSortedCol];
    else
        nSortType = LVEX_NOCASE; /* IVK */

    KSortInfo SortInfo;
    SortInfo.Ascending = bSortAscending;
    SortInfo.SortType = (unsigned char)nSortType;

    SortItems(Compare, SortInfo.lParam);
